...
- **Backwardation is the crisis state.** When spot VIX spikes above the futures, the curve inverts. In backwardation, the roll works *for* the long-vol ETPs and violently against short-vol positions. Feb 2018 (VIX 17→50 in days, several inverse-vol products terminated), Mar 2020, Aug 2024, Apr 2025 are the canonical stress windows for this repo.
...
- Each trading day it sells a slice of M1 and buys M2 so that the weighted-average maturity remains ~30 days. Over a full roll cycle, the entire position migrates M1→M2.
...
- **Daily roll yield ≈ (M1 − M2) / M2 × (1 / days-in-roll-period).** In typical contango of 5–10% between M1 and M2 over a ~21-trading-day cycle, this is roughly −5 to −10% per month of headwind for the ETP. This is arithmetic, not a forecast.
...
4. **Dew-Becker & Giglio, Chicago Fed WP 2025-17.** The reality check. Documents that equity index option returns and CAPM alphas — sharply negative historically (i.e., great for vol sellers) — have become statistically indistinguishable from zero over the past ~15 years. Attributes it to better-capitalized, more competitive vol sellers via an intermediary asset-pricing model. **Implication: the unconditional VRP is dead. The premium is now state-dependent — meaningfully positive when implied vol is rich vs. a realized-vol forecast and after vol shocks, ~zero otherwise.** This is why the strategy is a *filter*, and why doing nothing between alerts is the discipline, not a limitation.
...
- **HARVEST:** `ratio_vix_vix3m` < threshold θ₁ (start ~0.92) **and** `slope_m1m2` > θ₂ (start ~3%) **and** both stable/improving over a short lookback (e.g., 5-day slope of the ratio not rising). Alert: eligible to enter.
- **NEUTRAL:** contango present but shallow, or curve flattening. No new entries; manage existing per exit rules.
- **STRESS:** `ratio_vix_vix3m` ≥ 1.0 or VIX9D/VIX ≥ 1.0 or basis collapse > θ₃ within N days. No entries. Existing positions: exit per rules (their loss is already capped, but salvage value decisions are pre-specified, not improvised).
...
Thresholds θ are hypotheses. They get fit in-sample and validated walk-forward. **Never hand-tune thresholds on the full sample.**
...
- Long put near-the-money (delta ≈ −0.45 to −0.55 on entry)
- Short put at lower strike, roughly at the level implied by expected roll decay over the holding period (e.g., strike ≈ spot × (1 + expected decay to exit date))
- 30–60 DTE at entry
...
Working hypothesis to validate (NOT assume): p_win ≈ 0.60–0.70 in HARVEST regimes, avg_win ≈ +0.6 to +0.8R (due to early profit-taking), avg_loss ≈ −0.7 to −1.0R (regime exits salvage some premium; full −1R losses occur on gaps). That yields E ≈ +0.10 to +0.25R per trade before slippage. With ~10–20 qualifying trades/year and 1–2% of sleeve risked per trade, this reconciles to the mid-single-to-mid-teens annual expectation. **If Sprint 5 backtest expectancy falls below +0.05R net of modeled slippage, the strategy does not go live.**
...
3. Live phase: realized expectancy over first 20 trades < 0R, or a single position loses more than the debit (indicates a structural misunderstanding — should be impossible).
...
## 9. Data & Engineering Landmines (agent: read before writing ETL)
...
- **Roll yield:** P&L impact of the daily M1→M2 migration, sign-flipped for long ETPs in contango.
...
1. This file and `plan.md` are the spec. Propose amendments as diffs with rationale; never silently diverge.
...
6. Nothing in this repo constitutes advice to anyone else; it is personal research infrastructure. All go-live decisions are made by the human, at the gates defined in plan.md.

---

*Changelog:*
- *2026-07-11: v2 — full rewrite as agent-context document. Supersedes strategy.md from June session.*
